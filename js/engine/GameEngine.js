...
export class GameEngine {
  constructor(container, options = {}) {
    ...
    this.maxFood = 80; this.maxPower = 10; this.maxAI = 8;
  ...
  _initCamera() {
    ...
    this.camera = new THREE.PerspectiveCamera(this._baseFov, aspect, 0.1, 400);
  ...
  async init() {
    ...
    this._spawnFoods(); this.setState(GameState.MENU);
  }
  _loadWorld(worldId) {
  ...
  _spawnFoods() {
    while (this.foods.length < this.maxFood)
      this.foods.push(spawnFood(this.scene, Math.min(80, this.world.bounds * 0.4), this.foods, this.spawnOpts));
  }
  _spawnPowerUps() {
    while (this.powerUps.length < this.maxPower)
      this.powerUps.push(spawnPowerUp(this.scene, Math.min(90, this.world.bounds * 0.45), this.powerUps));
  }
  _spawnAIs() {
    this._clearAIs();
    for (let i = 0; i < this.maxAI; i++)
      this.ais.push(new AISnake(this.scene, { startLength: 5 + ((Math.random() * 8) | 0) }));
  }
  ...
  _burst(x, z, color = 0x3dffb5) {
    for (let i = 0; i < 5; i++) {
      ...
      m.position.set(x, 0.5, z); const a = Math.random() * Math.PI * 2; const sp = 2 + Math.random() * 3;
      this.scene.add(m);
  ...
  startGame() {
    ...
    let worldId = this.worldId; this.maxAI = 8; this.maxFood = 80; this.maxPower = 10;
    if (this.mode === 'campaign') {
      const lv = getLevel(this.levelId); worldId = lv.world || worldId;
      const m = lv.mods || {}; this.activeMods = { ...m };
      this.maxFood = m.foodMax || 60; this.maxAI = m.ais ?? 6;
    ...
    this._clearFoods(); this._clearPowerUps(); this._spawnFoods(); this._spawnPowerUps(); this._spawnAIs();
  ...
  _emitStatus() {
    ...
    this.onStatus({ bite: e.bite || 0, speed: e.speed || 0, shield: e.shield || 0, rivals: this.ais.filter((a) => a.alive).length });
  }
  ...
  _killAI(ai) {
    ...
    this.snake.addScore(Math.max(20, ai.length * 5)); this.onScore(this.snake.score, this.snake.combo);
    ai.die();
  }
  _checkGoal() {
  ...
  _updateGameplay(dt) {
    ...
    const lim = this.world.bounds - 2; const h = this.snake.segments[0];
    h.x = Math.max(-lim, Math.min(lim, h.x)); h.z = Math.max(-lim, Math.min(lim, h.z));
    h.mesh.position.set(h.x, 0.34, h.z);
    if (this.snake.checkSelfCollision() && !this.snake.hasShield()) { this.gameOver(); return; }
    for (const ai of this.ais) {
      if (!ai.alive) continue;
      ai.aiUpdate(dt, this.foods, this._headPos, this.snake.alive);
      const ah = ai.segments[0];
      ah.x = Math.max(-lim, Math.min(lim, ah.x)); ah.z = Math.max(-lim, Math.min(lim, ah.z));
      ah.mesh.position.set(ah.x, 0.34, ah.z);
      ...
      if (this._hitBody(this.snake, ai, 3)) {
        if (this.snake.hasBite()) {
          const stolen = ai.stealMass(3); this.snake.grow(stolen); this.snake.addScore(30 + stolen * 15);
      ...
      if (Math.hypot(this._headPos.x - this._aiHead.x, this._headPos.z - this._aiHead.z) < 0.55) {
        ...
        else if (!this.snake.hasShield()) { this.gameOver(); return; }
      }
    }
    for (let i = this.foods.length - 1; i >= 0; i--) {
      const f = this.foods[i]; if (!f.alive) continue;
      ...
      if (Math.hypot(this._headPos.x - p.x, this._headPos.z - p.z) < 0.72) {
        this.snake.addScore(f.value * 10); this.snake.grow(f.growAmount);
    ...
    for (let i = this.powerUps.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(this._headPos.x - p.x, this._headPos.z - p.z) < 0.85) {
        const got = pu.collect(); this.powerUps.splice(i, 1);
        if (got.type === 'star') { this.snake.addScore(50); this.goalProgress.stars++; this.onScore(this.snake.score, this.snake.combo); }
        else if (got.type === 'prize') { this.snake.addScore(100); this.snake.grow(4); this.onScore(this.snake.score, this.snake.combo); }
        else { this.snake.applyEffect(got.type, got.duration); if (got.type === 'speed') this.snake.speedMult = Math.max(this.snake.speedMult, 1.5); }
        this._burst(p.x, p.z, 0xffffff); this._emitStatus();
      }
    }
    this._spawnPowerUps();
    if (this.ais.filter((a) => a.alive).length < this.maxAI && Math.random() < 0.01)
      this.ais.push(new AISnake(this.scene, { startLength: 5 + ((Math.random() * 5) | 0) }));
    this._emitStatus();
    if (this.mode === 'campaign') {
  ...
  _updateCamera(dt) {
    ...
    const wantFov = this._baseFov + (this.snake.boosting ? 5 : 0) + Math.min(6, len * 0.05);
