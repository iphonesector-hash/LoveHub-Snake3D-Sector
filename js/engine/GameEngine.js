...
export class GameEngine {
  ...
  _updateGameplay(dt) {
    ...
    const obsHit = this.collision.testHead(this._headPos.x, this._headPos.z, 0.4);
    if (obsHit.hit) {
      if (obsHit.lethal) {
        if (this.snake.hasShield()) this.snake.effects.shield = 0;
    ...
    for (let aiIdx = 0; aiIdx < this.ais.length; aiIdx++) {
      ...
      // Player head → AI body => player dies (unless BITE/shield/ghost)
