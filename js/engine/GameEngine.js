...
export class GameEngine {
  ...
  _updateGameplay(dt) {
    ...
    if (colRes.absorbed) {
      this.onToast({ text: 'SHIELD BROKEN', color: '#60ffb0' });
    ...
    for (let i = this.foods.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(this._headPos.x - p.x, this._headPos.z - p.z) < 0.72) {
        ...
        this.onScore(this.snake.score, this.snake.combo);
        if (f.type === 'star') this.onToast({ text: 'STAR +' + (f.value * 10), color: '#ffe060' });
        else if (f.type === 'crystal') this.onToast({ text: 'CRYSTAL +' + (f.value * 10), color: '#40f0d0' });
      }
    }
    ...
    for (let i = this.loot.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(this._headPos.x - o._x, this._headPos.z - o._z) < 0.65) {
        ...
        if (got) {
          ...
          this.onToast({ text: 'LOOT +' + got.value, color: '#ffd060' });
        }
      }
    }
    ...
    for (let i = this.powerUps.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(this._headPos.x - p.x, this._headPos.z - p.z) < 0.85) {
        ...
        this.onToast({ text: toastText, color: toastColor });
        ...
        else if (got.type === 'mass_gain') { this.snake.grow(4); this.snake.addScore(40); this.onScore(this.snake.score, this.snake.combo); }
        else if (got.type === 'coin_rain') { this.snake.coins += 25; this.snake.addScore(60); this.onScore(this.snake.score, this.snake.combo); }
