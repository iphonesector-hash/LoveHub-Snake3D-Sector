...
export class GameEngine {
  ...
  gameOver() {
    ...
    this._burst(this.snake.segments[0].x, this.snake.segments[0].z, 0xff4060, 14);
    this._pendingGO = { score: this.snake.score, length: this.snake.length, mode: this.mode, levelId: this.levelId, xp: this.snake.xp, coins: this.snake.coins };
  }
  _finishGameOver() {
  ...
  _killSnake(ai, killer = null) {
    ...
    this._burst(hx, hz, ai.aiType === 'golden' ? 0xffd060 : 0xff4060, 10);
  ...
  _updateGameplay(dt) {
    ...
    if (colRes.killed) { this.gameOver(); return; }
    if (colRes.shake) this._shake = Math.max(this._shake, colRes.shake);
    if (colRes.nearMiss && this.nearMissCD <= 0) this.nearMissCD = 0.4;
    ...
    for (let aiIdx = 0; aiIdx < this.ais.length; aiIdx++) {
      ...
      if (this.nearMissCD <= 0) {
        ...
        if (dHead > 0.7 && dHead < 1.6) {
          ...
          this.onScore(this.snake.score, this.snake.combo);
          this.onToast({ text: 'NEAR MISS', color: '#80e0ff' });
        }
      }
    ...
    for (let i = this.chests.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(this._headPos.x - c._x, this._headPos.z - c._z) < 1.2) {
        ...
        if (reward) {
          ...
          this.onScore(this.snake.score, this.snake.combo);
          this.onToast({ text: reward.name, color: '#ffd060' });
    ...
    for (let i = this.powerUps.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(this._headPos.x - p.x, this._headPos.z - p.z) < 0.85) {
        ...
        if (got.type === 'star') { this.snake.addScore(50); this.goalProgress.stars++; this.onScore(this.snake.score, this.snake.combo); }
          ...
          const labels = { speed: 'SPEED +', shield: 'SHIELD', magnet: 'MAGNET', ghost: 'GHOST', bite: 'BITE', golden_bite: 'GOLDEN BITE', freeze: 'FREEZE', multiplier: 'x2 SCORE', double_xp: 'DOUBLE XP' };
          const t = labels[got.type];
          if (t) this.onToast({ text: t + (got.duration ? (' ' + Math.ceil(got.duration) + 's') : ''), color: '#a0ffe0' });
        }
